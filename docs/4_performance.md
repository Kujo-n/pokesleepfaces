...
### 8. コード分割と遅延読み込み (Code Splitting & Lazy Loading)

**目的**: 初期バンドルサイズの削減とロード時間の短縮

**実装場所**: `app/page.tsx`

**実装**:
- 使用頻度の低いモーダルコンポーネント（ヘルプ、収集状況）を `next/dynamic` で遅延読み込み
- ユーザーがアクションを起こすまでコンポーネントのコードを読み込まない
